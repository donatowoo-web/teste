...
async function run() {
  // Get all posts that have content but not body (migrated ones)
  const q = encodeURIComponent('*[_type=="post" && defined(content)] { _id, title, content }');
  const res = await fetch(`${API}/data/query/${DATASET}?query=${q}`, {
    headers: { Authorization: `Bearer ${TOKEN}` }
  });
  const data = await res.json();
  const posts = data.result || [];
  console.log(`Posts to restore body: ${posts.length}`);
  ...
  if (mutRes.ok) {
    console.log(`\nDone! ${posts.length} posts: body restored from content`);
  } else {
